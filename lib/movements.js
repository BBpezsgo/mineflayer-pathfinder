...
class Movements {
  ...
  */
  ...
  getBlock (pos, dx, dy, dz) {
    ...
    if (block.stateShapes) {
      ...
      if (block.stateShapes[block.metadata]) {
        // @ts-ignore
        shapes = block.stateShapes[block.metadata]
      } else {
        // @ts-ignore
        shapes = block.stateShapes[0]
      }
  ...
  safeOrBreak (block, toBreak) {
    ...
    const b = require('prismarine-block').Block.fromStateId(block.stateId, this.bot.blocks.biomeAt(block.position))
