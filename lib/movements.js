...
class Movements {
  ...
  dangerCost(node) {
    ...
    if (a0.name === 'air' && a1.name === 'air' && a2.name === 'air') { cost += 1 }
    if (b0.name === 'air' && b1.name === 'air' && b2.name === 'air') { cost += 1 }
    if (c0.name === 'air' && c1.name === 'air' && c2.name === 'air') { cost += 1 }
    if (d0.name === 'air' && d1.name === 'air' && d2.name === 'air') { cost += 1 }
  ...
  getBlock(pos, dx, dy, dz) {
    /** @type {import('..').SafeBlock | null} */ // @ts-ignore
   ...
   * @param {import('..').SafeBlock} block block
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    const blockGroundAfter = this.getBlock(node, dir.x, -1, dir.z)
    const jump = blockFootAfter.canWalkOn ? 1 : 0
  ...
  getMoveParkourForward(node, dir, neighbors) {
    ...
    const maxD = this.allowSprinting ? 5 : 2
