...
class Movements {
  ...
  constructor(bot, other = null) {
    ...
    if (bot.inventory.items()[bot.getEquipmentDestSlot('feet')]?.name === 'leather_boots') {
    } else {
      this.blocksToAvoid.add(registry.blocksByName['powder_snow'].id)
    }
    ...
    this.exclusionAreasStep.push(block => {
      if (block.type === registry.blocksByName.fire.id) {
        return 100
      }
      return 0
    })
  ...
  dangerCost(node) {
    ...
    if (a0.name === 'air' && a1.name === 'air' && a2.name === 'air') { cost += 30 }
    if (b0.name === 'air' && b1.name === 'air' && b2.name === 'air') { cost += 30 }
    if (c0.name === 'air' && c1.name === 'air' && c2.name === 'air') { cost += 30 }
    if (d0.name === 'air' && d1.name === 'air' && d2.name === 'air') { cost += 30 }
  ...
  getMoveJumpUp(node, dir, neighbors) {
    ...
    if (!isCarpetFence) {
      ...
      } else if (blockGroundAfter.height - blockGround.height > 1.2) {
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    if (!blockGroundAfter.physical) { return }
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    {
      const blockHeadAfter1 = this.getBlock(node, 0, jump + 1, dir.z)
      const blockFootAfter1 = this.getBlock(node, 0, jump, dir.z)
      const blockGroundAfter1 = this.getBlock(node, 0, jump - 1, dir.z)
    ...
    const toBreak2 = []
    {
      const blockHeadAfter2 = this.getBlock(node, dir.x, jump + 1, 0)
      const blockFootAfter2 = this.getBlock(node, dir.x, jump, 0)
      const blockGroundAfter2 = this.getBlock(node, dir.x, jump - 1, 0)
    ...
    if (this.getBlock(node, dir.x, jump - 1, dir.z).liquid) cost += this.liquidCost
    if (cost === Infinity) return
  ...
  getMoveUp(node, neighbors) {
    ...
    if (blockFoot.liquid || blockHead.liquid) {
      if (blockAbove.safe) {
        neighbors.push(new Move(node.x, node.y + 1, node.z, node.remainingBlocks, 1, [], []))
      }
      return
    }
