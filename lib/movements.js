...
class Movements {
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    // Open fence gates
    if (this.canOpenDoors && blockFootAfter.openable && blockFootAfter.shapes && blockFootAfter.shapes.length !== 0) {
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    const y = blockFootAfter.physical ? 1 : 0

    const blockFoot = this.getBlock(node, 0, -1, 0)
    ...
    if (blockD1.height - blockFoot.height > 1.2) cost1 += this.safeOrBreak(blockD1, toBreak1)
    ...
    if (blockD2.height - blockFoot.height > 1.2) cost2 += this.safeOrBreak(blockD2, toBreak2)
    ...
    if (y === 1) { // Case jump up by 1
      if (blockFootAfter.height - blockFoot.height > 1.2) return // Too high to jump
    ...
    } else if (blockD.physical || blockFootAfter.liquid) {
