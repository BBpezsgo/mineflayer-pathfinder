...
class Movements {
  ...
  /** @readonly @type {Set<number>} */ blocksCantBreak
  /** @readonly @type {Set<number>} */ blocksCanBreakAnyway
  /** @readonly @type {Set<number>} */ blocksToAvoid
  /** @readonly @type {Set<number>} */ liquids
  /** @readonly @type {Set<number>} */ gravityBlocks
  /** @readonly @type {Set<number>} */ climbables
  /** @readonly @type {Set<number>} */ emptyBlocks
  /** @readonly @type {Set<number>} */ replaceables
  /** @readonly @type {Set<number>} */ fences
  /** @readonly @type {Set<number>} */ carpets
  /** @readonly @type {Set<number>} */ openable
  /** @readonly @type {Array<number>} */ scafoldingBlocks

  /** @readonly @type {number} */ maxDropDown
  /** @readonly @type {boolean} */ infiniteLiquidDropdownDistance
  /** @type {number} */ digCost
   ...
   * ```js
    ...
    ```
    * @readonly @type {Array<(block: import('mineflayer-pathfinder').SafeBlock) => number>}
    */
  exclusionAreasStep
  ...
  getBlock(pos, dx, dy, dz) {
    ...
    b.physical = b.boundingBox === 'block' && !this.fences.has(b.type) && !this.emptyBlocks.has(b.type)
  ...
  getMoveJumpUp(node, dir, neighbors) {
    ...
    if (!blockGroundAfter.physical && blockGroundAfter.name !== 'end_portal') {
    ...
    if (blockGroundAfter.height - block0.height > 1.2) return // Too high to jump
  ...
  getMoveForward(node, dir, neighbors) {
    const blockHead = this.getBlock(node, dir.x, 1, dir.z)
    const blockFoot = this.getBlock(node, dir.x, 0, dir.z)
    const blockGround = this.getBlock(node, dir.x, -1, dir.z)

    let cost = 1 // move cost
    cost += this.exclusionStep(blockFoot)

    const toBreak = []
    const toPlace = []

    if (!blockGround.physical &&
        !blockFoot.liquid &&
        blockGround.name !== 'end_portal') {
      if (node.remainingBlocks === 0) return // not enough blocks to place

      if (this.getNumEntitiesAt(blockGround.position, 0, 0, 0) > 0) return // D intersects an entity hitbox

      if (!blockGround.replaceable) {
        if (!this.safeToBreak(blockGround)) return
        cost += this.exclusionBreak(blockGround)
        toBreak.push(blockGround.position)
      }
    ...
    cost += this.safeOrBreak(blockHead, toBreak)
    if (cost > 100) return

    // Open fence gates
    if (this.canOpenDoors && blockFoot.openable && blockFoot.shapes && blockFoot.shapes.length !== 0) {
      ...
      cost += this.safeOrBreak(blockFoot, toBreak)
      if (cost > 100) return
    }

    if (this.getBlock(node, 0, 0, 0).liquid) cost += this.liquidCost

    neighbors.push(new Move(blockFoot.position.x, blockFoot.position.y, blockFoot.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace))
  }
  ...
  getMoveParkourForward(node, dir, neighbors) {
    ...
    for (let d = 2; d <= maxD; d++) {
      ...
      if (ceilingClear && blockB.safe && blockC.safe && blockD.physical) {
        ...
        cost += this.landingCost(node, blockD)
        // Forward
        ...
        if (blockA.safe && d !== 4) { // 4 Blocks forward 1 block up is very difficult and fails often
          cost += this.exclusionStep(blockB)
          cost += this.landingCost(node, blockC)
        ...
        if (blockE.physical) {
          ...
          cost += this.landingCost(node, blockE)
