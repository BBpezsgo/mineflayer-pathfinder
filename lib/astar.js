...
function reconstructPath(node, debug) {
  ...
  while (node.parent) {
    ...
    // @ts-ignore
    debug.drawPoint(node.data.offset(0.5, 0, 0.5), [1, 1, 1])
  ...
  compute() {
    ...
    while (!this.openHeap.isEmpty()) {
      ...
      for (const neighborData of neighbors) {
        ...
        this.movements.bot.debug.drawPoint(neighborNode.data.offset(0.5, 0, 0.5), lerpColor([0, 1, 0], [1, 0, 0], neighborNode.h / 10))
