...
class GoalLookAtBlock extends Goal {
  /**
   * @param {Vec3} pos
   * @param {import('prismarine-world').world.WorldSync} world
   * @param {{
   *   reach?: number;
   *   entityHeight?: number;
   * }} [options]
  ...
  isEnd (node) {
    ...
    const validFaces = []
    for (const faceAxis of /** @type {Array<'x' | 'y' | 'z'>} */ (['x', 'y', 'z'])) {
...
class GoalBreakBlock extends Goal {
   ...
   * @param {any} [options]
   */
  constructor (x, y, z, world, options = {}) {
    super()
    this.goal = new GoalLookAtBlock(new Vec3(x, y, z), world, options)
  }
